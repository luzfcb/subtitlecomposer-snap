...
class Qmake2Plugin(qmake.QmakePlugin):
    ...
    def _extra_config(self):
        ...
        for root in [self.installdir, self.project.stage_dir]:
            ...
            extra_config.append("LIBS+=\"-L{}\"".format(self.project.parts_dir + '/qt57/install/lib'))
            extra_config.append("QMAKE_LIBS+=\"-L{}\"".format(self.project.parts_dir + '/qt57/install/lib'))
            extra_config.append("QMAKE_LIBDIR+=\"{}\"".format(self.project.parts_dir + '/qt57/install/lib'))

            paths = common.get_include_paths(root, self.project.arch_triplet)
            for path in paths:
                extra_config.append("INCLUDEPATH+=\"{}\"".format(path))
            extra_config.append("INCLUDEPATH+=\"{}\"".format(self.project.parts_dir + '/qt57/install/include'))

            extra_config.append("QML_IMPORT_PATH+=\"{}\"".format(self.project.parts_dir + '/qt57/install/qml'))
            extra_config.append("QML2_IMPORT_PATH+=\"{}\"".format(self.project.parts_dir + '/qt57/install/qml'))

        return extra_config

    def _build_environment(self):
        env = os.environ.copy()
        env['QTDIR' ] = self.project.parts_dir + '/qt57/install/'
        env['QML_IMPORT_PATH' ] = self.project.parts_dir + '/qt57/install/qml'
        env['QML2_IMPORT_PATH' ] = self.project.parts_dir + '/qt57/install/qml'
        env['LD_LIBRARY_PATH' ] = self.project.parts_dir + '/qt57/install/lib:' + self.project.parts_dir + '/qt57/install/usr/lib/:'
        env['PATH' ] = self.project.parts_dir + '/qt57/install/bin:' + os.environ["PATH"] 
        return env
